const debug = require('debug')('webpack:ssr')
...
module.exports = (
  ...
  // we just use babel-preset-gamma.
  ...
  let config = {
    ...
    // We need to tell webpack how to resolve both `gamma-core`'s node_modules and
  ...
  if (IS_NODE) {
    ...
    if (IS_DEV) {
      ...
      // config.entry.unshift('gamma-dev-utils/prettyNodeErrors')
    ...
    if (IS_DEV) {
      ...
      config.entry = {
        client: [
          ...
          // the default path. If you are doing some vendor bundling, you'll need to require the gamma/polyfills
          // on your own.
  ...
  // Apply gamma plugins, if they are present in gamma.config.js
  ...
  // Check if gamma.config has a modify function. If it does, call it on the
  // configs we created.
  if (modify) {
...
//         loader: 'babel-loader',
//         options: {
//           presets: ['babel-preset-gamma', 'react'],
//           plugins: [
