...
function build(previousFileSizes) {
  ...
  if (gamma.clearConsole === false || !!gamma.host || !!gamma.port) {
    logger.warn(`Specifying options \`port\`, \`host\`, and \`clearConsole\` in gamma.config.js has been deprecated. 
Please use a .env file instead.

${gamma.host !== 'localhost' && `HOST=${gamma.host}`}
${gamma.port !== '3000' && `PORT=${gamma.port}`}
`)
  }

  // Create our production webpack configurations and pass in gamma options.
  let clientConfig = createConfig('web', 'prod', gamma, webpack)
  let serverConfig = createConfig('node', 'prod', gamma, webpack)
