function runPlugin(plugin, config, { target, dev }, webpack) {
  ...
  const completePluginName = `gamma-plugin-${plugin.name}`

  // Try to find the plugin in node_modules
  const gammaPlugin = require(completePluginName)
  if (!gammaPlugin) {
    throw new Error(`Unable to find '${completePluginName}`)
  }

  return gammaPlugin(config, { target, dev }, webpack, plugin.options)
}
